...
Any of the template variables can be overridden by a command-line assignment. For example, 
if a local copy of the base installation image exists it can be referenced using a file URL. 
...
Execute this command to start or stop the Tomcat web container: `sudo systemctl {start|stop} tomcat-jsvc`. 
