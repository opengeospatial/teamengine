...
public class TECore implements Runnable {
	...
	Document prevLog = null; // Log document for current test from previous test execution (resume and retest modes
								// only)
	...
	public void execute_profile(String profileName, List<String> params, boolean required) throws Exception {
		...
		if (suite.getStartingTest().equals(test.getQName())) {
			...
			if (required) {
				throw new Exception("Error: Profile " + profileName + " is not a valid profile for session "
						+ sessionId + ".");
			}
		}
	}
	...
	public int executeTest(TestEntry test, XdmNode params, XPathContext context) throws Exception {
		...
		if (opts.getLogDir() != null) {
			logger = createLog();
			logger.println("<log>");
			logger.println("<starttest local-name=\"" + test.getLocalName() + "\" " + "prefix=\"" + test.getPrefix()
					+ "\" " + "namespace-uri=\"" + test.getNamespaceURI() + "\" " + "path=\"" + testPath + "\" "
			...
			if (test.usesContext()) {
				...
				if (kind == Type.ATTRIBUTE) {
					logger.print("<value " + contextNode.getDisplayName() + "=\"" + contextNode.getStringValue() + "\"");
					// TODO: set namespace
					logger.println("/>");
	...
	public synchronized void callTest(XPathContext context, String localName, String namespaceURI, NodeInfo params,
		...
		if (test == null) {
			throw new Exception("Test " + key + " could not be found in index.");
		}
	...
	public void repeatTest(XPathContext context, String localName, String NamespaceURI, NodeInfo params, String callId,
			int count, int pause) throws Exception {
		String key = "{" + NamespaceURI + "}" + localName;
		TestEntry test = index.getTest(key);
				...
				if (fe.getMinArgs() >= argCount && fe.getMaxArgs() <= argCount) {
					...
					for (int i = 0; i < argCount; i++) {
						...
						if (types[i].equals(String.class)) {
							...
							throw new Exception("Error: Function " + key + " uses unsupported Java type "
									+ types[i].toString());
						}
					}
					try {
						...
						if (fe.isInitialized()) {
							// String instkey = fe.getId() + "," + Integer.toString(fe.getMinArgs()) + "," +
	 ...
	 *             the exception
	 * 
	 *             <soap-request version="1.1|1.2" charset="UTF-8"> <url>http://blah</url> <action>Some-URI</action>
	 *             <headers> <header MutUnderstand="true" rely="true" role="http://etc"> <t:Transaction
	 *             xmlns:t="some-URI" >5</t:Transaction> </header> </headers> <body> <m:GetLastTradePrice
	 *             xmlns:m="Some-URI"> <symbol>DEF</symbol> </m:GetLastTradePrice> </body> <parsers:SOAPParser
	 *             return="content"> <parsers:XMLValidatingParser> <parsers:schemas> <parsers:schema
	 *             type="url">http://blah/schema.xsd</parsers:schema> </parsers:schemas> </parsers:XMLValidatingParser>
	 *             </parsers:SOAPParser> </soap-request>
	 */
	static public URLConnection build_soap_request(Node xml) throws Exception {
		String sUrl = null;
		String method = "POST";
		String charset = ((Element) xml).getAttribute("charset").equals("")
				? ((Element) xml).getAttribute("charset")
				: "UTF-8";
	...
	public URLConnection build_request(Node xml) throws Exception {
		...
		/*
		 * // Get URLConnection values InputStream is = uc.getInputStream(); byte[] respBytes =
		 * IOUtils.inputStreamToBytes(is); int respCode = ((HttpURLConnection) uc).getResponseCode(); String respMess =
		 * ((HttpURLConnection) uc).getResponseMessage(); Map respHeaders = ((HttpURLConnection) uc).getHeaderFields();
		 * 
		 * // Construct the HttpResponse (BasicHttpResponse) to send to parsers HttpVersion version = new
		 * HttpVersion(1,1); BasicStatusLine statusLine = new BasicStatusLine(version, respCode, respMess);
		 * BasicHttpResponse resp = new BasicHttpResponse(statusLine); Set respHeadersSet = respHeaders.keySet(); for(
		 * Iterator it = respHeadersSet.iterator(); it.hasNext(); ) { String name = (String) it.next(); List valueList =
		 * (List) respHeaders.get(name); String value = (String) valueList.get(0); if (name == null) continue;
		 * resp.addHeader(name, value); } HttpEntity entity = new ByteArrayEntity(respBytes); resp.setEntity(entity);
		 * 
		 * return resp;
		 */
		return uc;
	...
	/**
	 * Converts CTL input form data to generate a Swing-based or XHTML form and reports the results of processing the
	 * submitted form. The results document is produced in (web context) or {@link SwingForm.CustomFormView#submitData}.
	 * 
	 * @param ctlForm
	 *            a DOM Document representing a &lt;ctl:form&gt; element.
	 * @throws java.lang.Exception
	 * @return a DOM Document containing the resulting &lt;values&gt; element as the document element.
	 */
