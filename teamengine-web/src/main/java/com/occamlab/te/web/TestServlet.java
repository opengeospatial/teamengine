...
public class TestServlet extends HttpServlet {
        ...
        try {
            ...
            for (Entry<String, List<File>> sourceEntry : conf.getSources()
                ...
                for (File ctlFile : index.getDependencies()) {
                    ...
                    File indexFile = new File(new File(conf.getWorkDir(),
            ...
            /*
             * File scriptsDir = conf.getScriptsDir(); String sourcesNames[] =
             * scriptsDir.list(); for (int i = 0; i < sourcesNames.length; i++)
             * { String sourcesName = sourcesNames[i];
             * //System.out.println("TestServlet: " + sourcesName); SetupOptions
             * setupOpts = new SetupOptions();
             * setupOpts.setWorkDir(conf.getWorkDir());
             * setupOpts.setSourcesName(sourcesName); File sourcesDir = new
             * File(scriptsDir, sourcesName); File configFile = new
             * File(sourcesDir, "config.xml"); if (configFile.canRead()) { //
             * TODO: process config file // For now, presence of config.xml
             * means load ctl/main.xml or ctl/main.ctl File ctlDir = new
             * File(sourcesDir, "ctl"); File source = new File(ctlDir,
             * "main.xml"); if (source.canRead()) { setupOpts.addSource(source);
             * } else { setupOpts.addSource(new File(ctlDir, "main.ctl")); } }
             * else { setupOpts.addSource(new File(sourcesDir, "ctl")); } Index
             * index = Generator.generateXsl(setupOpts);
             * indexes.put(sourcesName, index); }
             */
            int cacheSize = 0;
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
         ...
         * request.getPathInfo().indexOf("/monitor/") >= 0) {
         * processMonitor(request, response, false); } else { process(request,
         * response); }
         */
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
         ...
         * processMonitor(request, response, true); } else { process(request,
         * response); }
         */
        process(request, response);
    }
    ...
    /*
     * public void processMonitor(HttpServletRequest request,
     * HttpServletResponse response, boolean post) throws ServletException { try
     * { String uri = request.getRequestURL().toString(); MonitorCall mc =
     * monitors.get(uri); TECore core = mc.getCore();
     * 
     * String url = mc.getUrl(); String queryString = request.getQueryString();
     * if (queryString != null) { if (url.contains("?")) { url += queryString; }
     * else { url += "?" + queryString; } }
     * 
     * HttpURLConnection huc = (HttpURLConnection)(new
     * URL(url).openConnection()); CachedHttpURLConnection uc = new
     * CachedHttpURLConnection(huc);
     * 
     * String method = request.getMethod(); uc.setRequestMethod(method);
     * uc.setDoInput(true); uc.setDoOutput(post);
     * 
     * Enumeration requestHeaders = request.getHeaderNames(); while
     * (requestHeaders.hasMoreElements()) { String key =
     * (String)requestHeaders.nextElement(); uc.setRequestProperty(key,
     * request.getHeader(key)); }
     * 
     * byte[] data = null; if (post) { ByteArrayOutputStream baos = new
     * ByteArrayOutputStream(); copy_stream(request.getInputStream(), baos);
     * data = baos.toByteArray(); ByteArrayInputStream bais = new
     * ByteArrayInputStream(data); copy_stream(bais, uc.getOutputStream()); }
     * 
     * Document doc = DB.newDocument(); Element eRequest =
     * encodeRequest(request, doc, data); Element parserInstruction =
     * mc.getParserInstruction(); Element eResponse = core.parse(uc,
     * parserInstruction);
     * 
     * Map<String, List<String>> responseHeaders = uc.getHeaderFields(); for
     * (Entry<String, List<String>> entry : responseHeaders.entrySet()) { if
     * (entry.getKey() != null && !entry.getKey().equals("")) { for (String
     * value: entry.getValue()) { response.setHeader(entry.getKey(), value); } }
     * }
     * 
     * if (mc.isPassThrough()) { identityTransformer.transform(new
     * DOMSource(eResponse), new StreamResult(response.getOutputStream())); }
     * else { copy_stream(uc.getInputStream(), response.getOutputStream()); }
     * 
     * if (mc.getCallId() != null) { identityTransformer.transform(new
     * DOMSource(mc.getParams()), new DOMResult(doc)); Element eParams =
     * DomUtils.getElementByTagName(doc, "params"); Element eReqParam =
     * doc.createElement("param"); eReqParam.setAttribute("local-name",
     * "request"); eReqParam.setAttribute("namespace-uri", "");
     * eReqParam.setAttribute("prefix", ""); eReqParam.setAttribute("type",
     * "node()"); Element eReqValue = doc.createElement("value");
     * eReqValue.appendChild(eRequest); eReqParam.appendChild(eReqValue);
     * eParams.appendChild(eReqParam); Element eRespParam =
     * doc.createElement("param"); eRespParam.setAttribute("local-name",
     * "response"); eRespParam.setAttribute("namespace-uri", "");
     * eRespParam.setAttribute("prefix", ""); eRespParam.setAttribute("type",
     * "node()"); Element eRespValue = doc.createElement("value");
     * identityTransformer.transform(new DOMSource(eResponse), new
     * DOMResult(eRespValue)); eRespParam.appendChild(eRespValue);
     * eParams.appendChild(eRespParam); net.sf.saxon.s9api.DocumentBuilder
     * builder = core.getEngine().getBuilder(); XdmNode paramsNode =
     * builder.build(new DOMSource(doc)); monitorCallSeq++; String callId =
     * mc.getCallId() + "_" + Integer.toString(monitorCallSeq);
     * core.callTest(mc.getContext(), mc.getLocalName(), mc.getNamespaceURI(),
     * paramsNode.getUnderlyingNode(), callId); } } catch (Throwable t) { throw
