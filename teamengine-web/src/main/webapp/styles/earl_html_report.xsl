	...
	<xsl:template match="/">
		<html>
			<body>
				...
				<!--  Strting point of the Report -->
				<xsl:for-each select="rdf:RDF/earl:TestRequirement">
			...
			<table border="1">
				<tr>
					<td bgcolor="#B2F0D1">
						...
						<xsl:value-of select="string-length($passCount)" />
					</td>
					<td bgcolor="#FFB2B2">
						Fail:
						...
						<xsl:value-of select="string-length($failCount)" />
					</td>
					<td bgcolor="#CCCCCE">
						Skip:
						...
						<xsl:value-of select="string-length($skipCount)" />
					</td>
				</tr>
			</table>
		</p>
