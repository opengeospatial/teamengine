...
/**
 * A Realm implementation that reads user information from an XML file located in a
 * sub-directory of the users directory. A sample representation is shown below.
 *
 * <pre>
 * &lt;user&gt;
 *   &lt;name&gt;p.fogg&lt;/name&gt;
 ...
 * </pre>
 * <p>
 * <strong>WARNING:</strong> This implementation is deprecated, since it only handles
...
public class UserFilesRealm extends RealmBase {
	...
	/**
	 * Sets the location of the root users directory. This is specified by the "root"
	 * attribute of the Realm element in the context definition.
	 * @param root A String specifying a directory location (TE_BASE/users).
	 */
	...
	private GenericPrincipal readPrincipal(String username) {
		...
		try {
			...
			// Fortify Mod: If we failed to read user info then there is no point in
			// continuing
			return null;
		}
	...
	GenericPrincipal createGenericPrincipal(String username, List<String> roles) {
		...
		try {
			...
			// Fortify Mod: if klass is not populated, then there is no point in going
			// forward
			return null;
		}
