	...
	private Source generateStatisticsReports() {
		...
		if (null == teBasePath || teBasePath.isEmpty() || !(new File(teBasePath).exists())) {
			throw new WebApplicationException(Response.serverError()
				.entity("TE_BASE directory does not exists.")
		...
		if (!configFile.exists()) {
			throw new WebApplicationException(Response.serverError()
				.entity("Config file does not exists in TE_BASE directory.")
				.type(MediaType.TEXT_PLAIN)
				.build());
		}
		...
		if (!usersDir.exists()) {
			throw new WebApplicationException(Response.serverError()
				.entity("User directory does not exists in TE_BASE directory.")
				.type(MediaType.TEXT_PLAIN)
				.build());
		}
		...
		for (Integer i = 0; i < standardList.getLength(); i++) {
			...
			for (Integer j = 0; j < versionList.getLength(); j++) {
				...
				// Number of users executed the test suite standard per month in current
				// year
				ArrayList<Long> numberOfUsersExecutedTestSuitePerMonth = TEStatisticsUtil
					.numberOfUsersExecutedTestSuitePerMonth(testSuiteName, userDetails);

				// Number of test suite run per month in current year.
				ArrayList<Long> testSuiteRunPerMonth = TEStatisticsUtil.testSuiteRunPerMonth(testSuiteName,
						userDetails);

				// Number of test suite status(success, failure, Incomplete) run per
				// month.
				Map<String, Integer> testStatus = new HashMap<>();
				testStatus.put("Success", 1);
				testStatus.put("Failure", 6);
				testStatus.put("Incomplete", 0);
				Map<String, ArrayList<Long>> testSuiteStatusPerMonth = TEStatisticsUtil
					.testSuiteStatusPerMonth(testSuiteName, userDetails, testStatus);
				...
				Map<String, Object> testSuiteStatusWithDrilldown = TEStatisticsUtil
					.testSuiteStatusWithDrilldown(testSuiteName, userDetails, testStatus);
				...
				// Tests run per test suite in current year
				Map<String, Object> testSuiteRunDetails = TEStatisticsUtil
					.testSuiteRunDetailsOfCurrentYear(testSuiteName, userDetails);
