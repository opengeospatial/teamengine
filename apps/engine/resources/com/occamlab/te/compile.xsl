...
<!--			<xsl:copy-of select="namespace::*[name()=$prefix]"/> -->
			...
			<txsl:variable name="te:start-test">
				<starttest local-name="{$local-name}" prefix="{$prefix}" namespace-uri="{$namespace-uri}" file="{$filename}">
					<txsl:attribute name="path"><txsl:value-of select="$te:call-path"/></txsl:attribute>
					<assertion>
						<txsl:variable name="te:assertion">
							<xsl:for-each select="ctl:param[contains(../ctl:assertion, concat('{$', @name, '}'))]">
				...
				<endtest>
					<txsl:attribute name="result">
						<txsl:choose>
							...
							<txsl:otherwise>
								<txsl:value-of select="{concat('te:message($te:core, $te:call-depth, ', $apos, 'Test ', @name, ' Passed', $apos, ')')}"/>
	...
	<xsl:template match="ctl:form">
		...
		<txsl:variable name="te:form-call-id">
			<xsl:call-template name="loc"/>
			<xsl:attribute name="select"><xsl:value-of select="concat('concat(', $apos, $form-id, '_', $apos, ', position())')"/></xsl:attribute>
		</txsl:variable>
		...
		<txsl:variable name="te:formresults">
			<txsl:choose>
				...
				<txsl:otherwise>
					<formresults>
						<txsl:attribute name="id"><txsl:value-of select="$te:form-call-id"/></txsl:attribute>
...
<!-- 			<xsl:attribute name="select"><xsl:value-of select="concat('concat(', $apos, $request-id, '_', $apos, ', position())')"/></xsl:attribute> -->
	...
	<xsl:template match="ctl:parse">
		...
		<txsl:variable name="te:parse-id">
			<xsl:call-template name="loc"/>
			<xsl:attribute name="select"><xsl:value-of select="concat('concat(', $apos, $id, '_', $apos, ', position())')"/></xsl:attribute>
		</txsl:variable>
	...
	<xsl:template match="ctl:call-test">
		...
		<xsl:variable name="local-name" select="substring-after(@name, ':')"/>
		<xsl:variable name="namespace-uri" select="namespace::*[name()=$prefix]"/>
		<txsl:variable name="te:new-call-path" select="concat($te:call-path, '/{generate-id()}_', position())"/>
			...
			<txsl:otherwise>
				...
				<txsl:call-template name="{$prefix}:{$local-name}">
					...
					<xsl:for-each select="*[local-name()='with-param']">
						<txsl:with-param name="{@name}" select="$te:with-param-{position()}/value/node()|$te:with-param-{position()}/value/@*"/>
...
<!--			<xsl:copy-of select="namespace::*[name()=$prefix]"/> -->
			...
			<xsl:choose>
				<xsl:when test="ctl:code">
					...
					<xsl:apply-templates select="ctl:code/*[not(namespace-uri() = 'http://www.w3.org/1999/XSL/Transform' and local-name() = 'param')]" mode="fn-code"/>
				</xsl:when>
		...
		<txsl:transform version="1.0" exclude-result-prefixes="ctl saxon">
			<xsl:for-each select="//ctl:function|//ctl:parser">
				...
				<xsl:for-each select="ctl:java[@initialized='true']">
					...
					<txsl:variable name="{$prefix}:{$local-name}" select="initializer:new({substring($param-list,2)})">
						<xsl:call-template name="loc"/>
						<xsl:call-template name="namespace-attribute">
							<xsl:with-param name="prefix" select="$prefix"/>
							<xsl:with-param name="uri" select="$namespace-uri"/>
			...
			<xsl:for-each select="//ctl:parser">
				...
				<xsl:choose>
					<xsl:when test="ctl:java/@initialized = 'true'">
						<txsl:value-of select="te:register_parser($te:core, '{$namespace-uri}', '{$local-name}', '{ctl:java/@method}', ${@name})">
							<xsl:call-template name="loc"/>
							<xsl:copy-of select="namespace::*[name()=$prefix]"/>
						</txsl:value-of>
					</xsl:when>
					<xsl:otherwise>
						<txsl:value-of select="te:register_parser($te:core, '{$namespace-uri}', '{$local-name}', '{ctl:java/@method}', '{ctl:java/@class}')">
							<xsl:call-template name="loc"/>
							<xsl:copy-of select="namespace::*[name()=$prefix]"/>
						</txsl:value-of>
					</xsl:otherwise>
