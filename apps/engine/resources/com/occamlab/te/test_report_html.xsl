...
-->
    ...
    <xsl:template name="tableoc">
        ...
        <h2>
            <xsl:value-of select="concat(concat(/execution/log[1]/starttest/@prefix,':'),/execution/log[1]/starttest/@local-name)"/>
        </h2>
        <P>
            <xsl:if test="/execution/log[1]/endtest/@result &gt; 1">Failure</xsl:if>
            <xsl:if test="/execution/log[1]/endtest/@result = 1">Success with warnings</xsl:if>
            <xsl:if test="/execution/log[1]/endtest/@result = 0">Success</xsl:if>            
        </P>
        ...
        <xsl:if test="count(testcall) &gt; 0">
            <xsl:element name="ol">
                <xsl:for-each select="testcall">
                    ...
                    <li>
                        <xsl:element name="a">
                            <xsl:attribute name="href"><xsl:value-of select="concat('#',$sessione)"></xsl:value-of></xsl:attribute>
    ...
    <xsl:template name="ricorsione">
        ...
        <xsl:comment>
            RESULT = <xsl:value-of select="/execution/log/starttest[@path=$sessione2]/parent::log/endtest/@result"/>
        </xsl:comment>
        <xsl:if test="/execution/log/starttest[@path=$sessione2]/parent::log/endtest/@result &gt; 1">Failure</xsl:if>
        <xsl:if test="/execution/log/starttest[@path=$sessione2]/parent::log/endtest/@result = 1">Success with warnings</xsl:if>
        <xsl:if test="/execution/log/starttest[@path=$sessione2]/parent::log/endtest/@result = 0">Success</xsl:if>
    ...
    <xsl:template match="starttest" mode="call">
        <h2>
            <xsl:element name="a">
                ...
                test: <xsl:value-of select="concat(concat(./@prefix,':'),./@local-name)"/>
            </xsl:element>
        </h2>
        <code>session: <xsl:value-of select="./@path"/></code>
        <p><b>Assertion: </b> <xsl:value-of select="./assertion"/></p>         
    </xsl:template>
    ...
    <xsl:template match="testcall"  mode="call">
        ...
        <li>
            <xsl:element name="a">
            <xsl:attribute name="href"><xsl:value-of select="concat('#',$sessione3)"/>                
            </xsl:attribute>
                <xsl:value-of select="concat(concat(/execution/log/starttest[@path=$sessione3]/@prefix,':'),/execution/log/starttest[@path=$sessione3]/@local-name)"/>
        </xsl:element>
            <xsl:if test="/execution/log/starttest[@path=$sessione3]/parent::log/endtest/@result &gt; 1"> Failure</xsl:if>
            <xsl:if test="/execution/log/starttest[@path=$sessione3]/parent::log/endtest/@result = 1"> Success with warnings</xsl:if> 
            <xsl:if test="/execution/log/starttest[@path=$sessione3]/parent::log/endtest/@result = 0"> Success</xsl:if> 
            </li>
    </xsl:template>
    <xsl:template match="endtest"  mode="call">
        <xsl:comment>RESULT number=<xsl:value-of select="@result"/></xsl:comment>        
       <p> <b>Test result: </b> <xsl:if test="@result &gt; 1">Failure</xsl:if>
           <xsl:if test="@result = 1">Success with warnings</xsl:if>     <xsl:if test="@result = 0">Success</xsl:if>      
       </p> 
    </xsl:template>
    <xsl:template match="message"  mode="call">        
        <p class="message">Message <pre><xsl:value-of select="current()"/></pre></p>
    </xsl:template>
    ...
    <xsl:variable name="nl"><xsl:text>&#10;</xsl:text></xsl:variable>
        ...
        <xsl:template name="write-starttag">
            ...
            <xsl:if test="not(*|text()|comment()|processing-instruction())">
                /</xsl:if>
            <xsl:text>></xsl:text>
        </xsl:template>
        ...
        <xsl:template name="write-namespace-declarations">
            ...
            <xsl:for-each select="namespace::*">
                ...
                <xsl:if test="not(contains(concat(' ', $exclude-prefixes, ' xml '),
                    concat(' ', $ns-prefix, '')))
                    and ($is-top = 'true' or not($parent-nss[name() =
                    ...
                    <xsl:value-of select="concat($nl, $indent-string,
                        $ns-decl-extra-indent)" />
                    <xsl:choose>
                        <xsl:when test="$ns-prefix = ''">
                            <xsl:value-of 
                                select="concat('xmlns=&quot;', $ns-uri, '&quot;')"
                            />
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of 
                                select="concat('xmlns:', $ns-prefix, '=&quot;',
                                $ns-uri, '&quot;')" />
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:if>
