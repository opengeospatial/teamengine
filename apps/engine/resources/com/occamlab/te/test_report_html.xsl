...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:ctl="http://www.occamlab.com/ctl">
	<!--
...
-->
					...
					<li>
						<xsl:element name="a">
							<xsl:attribute name="href">
								<xsl:value-of select="concat('#',$sessione)"></xsl:value-of>
							</xsl:attribute>
	...
	<xsl:template name="ricorsione">
		...
		<xsl:comment>RESULT = 
			<xsl:value-of select="/execution/log/starttest[@path=$sessione2]/parent::log/endtest/@result"/>
		</xsl:comment>
	...
	<xsl:template match="starttest" mode="call">
		<h2>
			<xsl:element name="a">
				...
				<xsl:value-of select="concat(concat(./@prefix,':'),./@local-name)"/>
			</xsl:element>
		</h2>
		<code>session: 
			<xsl:value-of select="./@path"/>
	...
	<xsl:template match="endtest" mode="call">
		<xsl:comment>RESULT number=
			<xsl:value-of select="@result"/>
		</xsl:comment>
		<p>
			<b>Test result:</b>
			<xsl:if test="@result &gt; 1">Failure</xsl:if>
			<xsl:if test="@result = 1">Success with warnings</xsl:if>
			<xsl:if test="@result = 0">Success</xsl:if>
		</p>
	</xsl:template>
	...
	<xsl:template match="request" mode="call">
		<p>Submitted request:
			...
			<xsl:if test="count(current()/ctl:request/ctl:body/*) &lt; 1">empty.</xsl:if>
		</p>
		<pre>
	...
	<xsl:template name="write-namespace-declarations">
		...
		<xsl:for-each select="namespace::*">
			...
			<xsl:if test="not(contains(concat(' ', $exclude-prefixes, ' xml '), concat(' ', $ns-prefix, ''))) and ($is-top = 'true' or not($parent-nss[name() = $ns-prefix and string(.) = $ns-uri])) ">
