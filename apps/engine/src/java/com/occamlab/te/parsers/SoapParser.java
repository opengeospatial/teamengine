...
/**
 * Parse a SOAP message returning back the SOAP mssage itself or the content of the SOAP Body element.
 *
 */
public class SoapParser {
     ...
     *
     * @return null if there were errors, the parsed document otherwise:
     *         it can be the SOAP message, the SOAP message body content or a SOAP fult
     *
    ...
    public Document parse(URLConnection uc, Element instruction,
        ...
        if (responsecode == 200) {
             ...
             * (a) no response envelope is provided or
             * (b) an envelope representing information related to the request is provided
             */
            soapMessage = huc.getInputStream();
            ...
            /*
             * The requested resource has moved and the HTTP request SHOULD be retried using the URI carried in the
             * associated Location header field as the new value for the http://www.w3.org/2003/05/soap/mep/ImmediateDestination property.
             * The value of http://www.w3.org/2003/05/soap/features/web-method/Method is changed to ""GET"", the value of
             * http://www.w3.org/2003/05/soap/mep /OutboundMessage is set to "null".
             * [Note: Status code 303 MUST NOT be sent unless the request SOAP envelope has been processed according to the SOAP
             * processing model and the SOAP response is to be made available by retrieval from the URI provided with the 303.]
             */
            soapMessage = huc.getErrorStream();
     ...
     *
     * @param xml
     *            the SOAP message to retrieve and validate.  May be an InputStream object
    ...
    private Document parse(Object xml, Element instruction, PrintWriter logger) throws Exception {
        ...
        String returnType = instruction.getAttribute("return");//envelope or content
     ...
     * the propoer SOAP Schema (1.1 or 1.2 depending on the namespace of the incoming message)
     *
     * @param soapMessage
    ...
    /**
     * A method to parse a SOAP fault. It checks the namespace
     * and invoke the correct SOAP 1.1 or 1.2 Fault parser.
     *
     * @param soapMessage
