...
public class Test {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].startsWith("-cmd=")) {
				...
				if (f.exists()) {
					...
					addPath(f); // to add class path of given test LB 20121005
				} else {
	...
	// add to the classpath the directory provide in the command line / LB
	// 20121005
	public static void addPath(File f) throws Exception {
		f = f.getParentFile();
		// in the case that a particular name of a test is given, instead of the folder "ctl"
		if (f.getName().endsWith("ctl")){
			f = f.getParentFile();
		}
		// now f should be the parent folder of ctl and "resources"
