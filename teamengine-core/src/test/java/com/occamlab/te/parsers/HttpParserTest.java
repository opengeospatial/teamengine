...
public class HttpParserTest {
	...
	public static void setUpClass() throws Exception {
		File indexFile = new File(HttpParserTest.class.getResource("/conf/index-parsers.xml").toURI());
		Index index = new Index(indexFile);
		RuntimeOptions opts = new RuntimeOptions();
		Engine engine = new Engine();
		engine.setClassLoader("default", new TEClassLoader(new File(System.getProperty("user.home"))));
	...
	public void parseXmlEntityWithNullInstruction() throws Throwable {
		...
		assertNotNull("Expected article element.", rsp.getElementsByTagNameNS(DOCBOOK_NS, "article").item(0));
	...
	public void parseTextEntityWithNullInstruction() throws Throwable {
		...
		Element content = (Element) result.getElementsByTagName("content").item(0);
		assertNotNull("Expected content element.", content);
		assertTrue("Expected text content starting with 'Twas brillig",
				content.getTextContent().startsWith("'Twas brillig"));
	}
	...
	public void parseTextEntityWithImageParser() throws Throwable {
		URL url = this.getClass().getResource("/jabberwocky.txt");
		URLConnection urlConn = url.openConnection();
		Document imgParser = docBuilder.parse(getClass().getResourceAsStream("/conf/HttpParser+ImageParser.xml"));
		Document result = HTTPParser.parse(urlConn, imgParser.getDocumentElement(), logger, teCore);
		Element content = (Element) result.getElementsByTagName("content").item(0);
		assertTrue("content element should have no child nodes", !content.hasChildNodes());
	}

	static String writeNodeToString(Node node)
