 ...
 *
 * The specific rules applied by TEPath are: 1) The path is restricted to one of the
 * following root directories a) TE_BASE - path defined by the TE_BASE environment
 * variable b) TE_INSTALL - path defined by the TE_INSTALL environment variable c) Java
 * temp directory d) TE_BUILD - An environment variable which contains the path to this
 * source tree 2) No navigating up the directory tree 3) These paths are never allowed a)
 * c:\\Windows b) /etc c) /bin d) /usr/bin
 *
...
public class TEPathTest {
	...
	public static void initialize() {
		...
		if (te_base == null)
			te_base = System.getProperty("java.io.tmpdir");
		String tmpdir = System.getProperty("java.io.tmpdir");
		if (tmpdir == null)
			tmpdir = te_base;
		String te_install = System.getProperty("TE_INSTALL");
		if (te_install == null)
			te_install = System.getenv("TE_INSTALL");
		if (te_install == null)
			te_install = te_base;
		...
		if (separator.equals("/")) {
			...
			invalidpath1 = validpath1.concat("/../etc"); // Navigation up the tree is not
															// allowed
		...
		if (separator.equals("\\")) {
			validpath5 = validpath1.concat("\\foo-bar"); // Valid path name
			invalidpath1 = validpath4.concat("\\..\\Windows"); // Navigation up the tree
																// is not allowed
