...
public class RecordTestResult {
	...
	/**
	 * Check Test recording ON or OFF if ON then create a builder for storing the log in
	 * single file.
	 * @param suite
	 * @throws Exception
	 */
	...
	/**
	 * Save all recording into file. If the dirPath argument is null, then don't record.
	 * @param suite
	 * @param dirPath
	 * @throws Exception
	 */
	...
	public void saveRecordingData(SuiteEntry suite, File dirPath) throws Exception {
		...
		if (dirPath != null) {
			...
			if (!tpath.isValid())
				throw new IllegalArgumentException(
						"Invalid argument to saveRecordingData: dirPath = " + dirPath.getAbsolutePath());
		}
		if (dirPath != null && null != suite && SetupOptions.recordingInfo(suite.getLocalName())) {
			...
			try {
				...
				OutputStream report_logs = new FileOutputStream(
						new File(dirPath.getAbsolutePath() + Constants.tmp_File));
				...
				bufferedWriter = new BufferedWriter(
						new FileWriter(dirPath.getAbsolutePath() + Constants.result_logxml));
				String dataString = "";
				...
				// Fortify Mod: Free up the Buffered Reader and Writer and their
	...
	/**
	 * Save recording clause into file. If the dirPath argument is null, then don't
	 * record.
	 * @param suite
	 * @param dirPath
	 * @throws Exception
	...
	public void saveRecordingClause(SuiteEntry suite, File dirPath) throws Exception {
		...
		if (dirPath != null && null != suite && SetupOptions.recordingInfo(suite.getLocalName())) {
			try {
				...
				// Create a xml file for saving the data.
				bufferedWriter = new BufferedWriter(
						new FileWriter(dirPath.getAbsolutePath() + Constants.result_clausexml));
				String dataString = "";
				// Read the data from file.
				while ((dataString = bufferedReader.readLine()) != null) {
					// Replace special symbol code to its symbol
	...
	/**
	 * Store start test detail into file. Recording is disabled if dirPath argument is
	 * null
	 * @param test
	 * @param dirPath
		...
		if (dirPath != null && A_TRUE.equals(System.getProperty(RECORD))) {
			...
			if (TECore.testCount != 0) {
				...
				OutputStreamWriter writerBefore = new OutputStreamWriter(
						new FileOutputStream(dirPath.getAbsolutePath() + Constants.TEST_RESULTTXT, true),
	...
	/**
	 * Save test detail into file. If the dirPath parameter is null, then don't record
	 * @param test
	 * @param verdict
	 * @param dirPath
	 * @param cal
	 ...
	 * @throws java.lang.Exception
	 */
	public void storeFinalTestDetail(TestEntry test, DateFormat dateFormat, Calendar cal, File dirPath)
			throws Exception {
		// Fortify Mod: make sure that dirPath is a legal path
