...
public class Soap12MessageBuilder {
	...
	/**
	 * A method to get the SOAP 1.2 message. The message is created and returned as DOM
	 * Document. author Simone Gianfranceschi
	 * @param headerBlocks The list of header blocks to be included in the SOAP message
	 * header
	 * @param i_body The XML file to be inclued in the SOAP message body.
						...
						if (children.item(i).getNodeType() == Node.ELEMENT_NODE) {
							...
							for (int k = 0; k <= blockAttributes.getLength() - 1; k++) {
								...
								if (attributeName.equals(RELY) || attributeName.equals(MUST_UNDERSTAND)) {
									...
									((Element) importedNode).setAttributeNS(SOAP_NS_URI, SOAP_ENV + ":" + attributeName,
											attribute.getValue());
									...
									if (value.equals(ROLE_NEXT_SC)) {
										...
										// importedNode).setAttribute(SOAP_ENV +
										// ":" + ROLE, ROLE_NEXT);
										((Element) importedNode).setAttributeNS(SOAP_NS_URI, SOAP_ENV + ":" + ROLE,
												ROLE_NEXT);
									...
									else if (value.equals(ROLE_ULTIMATE_RECEIVER_SC)) {
										// ((Element)
										// importedNode).setAttribute(SOAP_ENV +
										// ":" + ROLE, ROLE_ULTIMATE_RECEIVER);
										((Element) importedNode).setAttributeNS(SOAP_NS_URI, SOAP_ENV + ":" + ROLE,
										...
										// importedNode).setAttribute(SOAP_ENV +
										// ":" + attributeName, value);
										((Element) importedNode).setAttributeNS(SOAP_NS_URI,
												SOAP_ENV + ":" + attributeName, value);
