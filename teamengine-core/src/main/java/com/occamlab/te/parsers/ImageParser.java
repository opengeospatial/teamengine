...
/**
 * Extracts the body of a response message and treats it as an image resource. If the
 * entity does not correspond to a supported image format, an exception is reported in the
 * test log.
 *
 */
public class ImageParser {
	...
	private static String getBase64Data(BufferedImage buffImage, String formatName, Node node) throws Exception {
		...
		if (!status) {
			throw new Exception(
					"No suitable ImageIO writer found by ImageParser.getBase64Data() for image format " + formatName);
		}
	...
	/*
	 * Process buffered image obtained from Reader 2011-09-08 PwD added formatName param
	 * to support getBase64Data()
	 */
		...
		while (node != null) {
			if (node.getNodeType() == Node.ELEMENT_NODE && node.getLocalName().equals("count")) {
				...
				if (sample.equals("all")) {
					...
					// Fortify Mod: If the Iterator sampleIt is empty, then countnode will
					// never be set.
					// This will result in a null derefernce of node at the end of the
					// loop. Test for this
	...
	/**
	 * Determines the width of the first image in an image file in pixels. author Paul
	 * Daisey added 2011-05-13 to support WMTS ETS
	 * @param imageLoc the string location of the image (uri syntax expected)
	...
	/**
	 * Determines the height of the first image in an image file in pixels. author Paul
	 * Daisey added 2011-05-13 to support WMTS ETS
	 * @param imageLoc the string location of the image (uri syntax expected)
	 * @return int the image width in pixels, or -1 if unable.
	 */
	...
	public static Document parse(URLConnection uc, Element instruction, PrintWriter logger)
		...
		try {
			...
			String msg = String.format("Failed to parse %s resource from %s %n %s", uc.getContentType(), uc.getURL(),
					e.getMessage());
			jlogger.warning(msg);
		}
		finally {
