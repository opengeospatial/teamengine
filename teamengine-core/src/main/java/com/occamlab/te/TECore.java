...
public class TECore implements Runnable {
	...
	public int executeTest(TestEntry test, XdmNode params, XPathContext context) throws Exception {
		...
		if (opts.getLogDir() != null) {
			...
			logger.print("defaultResult=\"" + test.getDefaultResult() + "\" ");
		...
		try {
			...
			// Add missing info in the log.xml E.g. endtag '</log> or' endtest '<endtest
			// result="1" />'.
	 ...
	 *
	 * if (logger != null) { logger.println("<testcall path=\"" + testPath + "/" + callId
	 * + "\"/>"); logger.flush(); } if (opts.getMode() == Test.RESUME_MODE) { Document doc
