...
public class ConfigFileBuilder {
	...
	public static void main(String[] args) throws Exception {
		...
		SortedSet<ConfigEntry> configs = new TreeSet<>(new ConfigComparator());
		...
		while (config != null) {
			...
			while (config != null && config.organization.equals(organization)) {
				...
				while (config != null && config.organization.equals(organization) && config.standard.equals(standard)) {
					...
					System.out
						.println("            <namespace-uri>" + config.suite.getNamespaceURI() + "</namespace-uri>");
					...
					while (config != null && config.organization.equals(organization)
						...
						if (config != null) {
							...
							for (int i = 0; i < profiles.size(); i++) {
								...
								System.out.println("              <namespace-uri>" + profiles.get(i).getNamespaceURI()
										+ "</namespace-uri>");
								System.out
									.println("              <prefix>" + profiles.get(i).getPrefix() + "</prefix>");
								System.out.println("              <local-name>" + profiles.get(i).getLocalPart()
										+ "</local-name>");
								...
								if (profileDescription != null) {
									System.out
										.println("              <description>" + profileDescription + "</description>");
								}
								System.out.println("            </profile>");
							}
							System.out.println("          </revision>");
