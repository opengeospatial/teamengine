...
public class ConfigFileBuilder {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String defaultRevision = (new SimpleDateFormat("yyyy-MM-dd"))
                .format(new Date());
            ...
            while (config != null && config.organization.equals(organization)) {
                ...
                while (config != null
                    ...
                    System.out.println("            <prefix>"
                            + config.suite.getPrefix() + "</prefix>");
                    System.out.println("            <local-name>"
                            + config.suite.getLocalPart() + "</local-name>");
                    String title = config.title;
                    if (title == null) {
                        title = organization + " " + standard + " " + version
                                + " Test Suite";
                    }
                    ...
                    if (config.link != null) {
                        System.out.println("            <link>" + config.link
                                + "</link>");
                    }
                    if (config.dataLink != null) {
                        System.out.println("            <link type=\"data\">"
                    ...
                    while (config != null
                        ...
                        while (config != null && config.suite == null) {
                            ...
                            profileDescriptions
                                    .addAll(config.profileDescriptions);
                        ...
                        if (config != null) {
                            ...
                            System.out.println("            <name>" + revision
                                    + "</name>");
                            System.out.println("            <sources>");
                            for (File source : sources) {
                                String path = source.getPath().substring(2)
                                        .replace('\\', '/');
                            ...
                            if (config.resources != null) {
                                String path = config.resources.getPath()
                                        .substring(2).replace('\\', '/');
                            ...
                            for (int i = 0; i < profiles.size(); i++) {
                                ...
                                System.out
                                        .println("              <namespace-uri>"
                                                ...
                                                + "</namespace-uri>");
                                System.out.println("              <prefix>"
                                        + profiles.get(i).getPrefix()
                                        + "</prefix>");
                                System.out.println("              <local-name>"
                                        + profiles.get(i).getLocalPart()
                                        + "</local-name>");
                                String profileTitle = profileTitles.get(i);
                                if (profileTitle == null)
                                    profileTitle = profiles.get(i)
                                            .getLocalPart();
                                System.out.println("              <title>"
                                        + profileTitle + "</title>");
                                String profileDescription = profileDescriptions
                                        .get(i);
